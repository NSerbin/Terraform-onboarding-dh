...
resource "aws_elasticache_cluster" "obn_devops_nserbin_sbx_redis" {
...
resource "aws_iam_role" "obn_devops_nserbin_sbx_db_ssm" {
  name               = "obn_devops_nserbin_sbx_db_ssm"
  assume_role_policy = file("template/ec2_assume_policy.json")
}

resource "aws_iam_role_policy_attachment" "vaulty_policy_attach_SSM" {
  role       = aws_iam_role.obn_devops_nserbin_sbx_db_ssm.name
  policy_arn = "arn:aws:iam::aws:policy/AmazonSSMManagedInstanceCore"
}

resource "aws_iam_instance_profile" "obn_devops_nserbin_sbx_db_ssm" {
  name = aws_iam_role.obn_devops_nserbin_sbx_db_ssm.name
  role = aws_iam_role.obn_devops_nserbin_sbx_db_ssm.name
}